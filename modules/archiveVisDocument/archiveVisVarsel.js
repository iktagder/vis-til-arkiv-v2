...
const findVarselData = require("../../modules/archiveVisDocument/findVisVarselData");
...
module.exports = async (archiveMethod, options, test=false) => {
    ...
    for (pdf of listOfPdfs) {
        ...
        let documentData = {};
        try {
            documentData = findVarselData(archiveMethod, pdfContent);
        ...
        let dsfStudent;
        try {
            let studentNameList = documentData.studentName.split(" ")
            let studentBirthdateList = documentData.studentBirthdate.split("/")
            const studentPerson = {
                birthdate: studentBirthdateList[0]+studentBirthdateList[1]+(studentBirthdateList[2].substring(2,4)),
                firstName: studentNameList[0],
                lastName: studentNameList[studentNameList.length-1]
            }
            dsfStudent = await dsfGetElev(undefined, archiveMethod.sendToParents, true, studentPerson)
            writeLog("  Found student in DSF, birthnr: "+dsfStudent.birthnr)
            documentData.studentBirthnr = dsfStudent.birthnr
        ...
        if (archiveMethod.sendToStudent && !blockedAddress && !archiveMethod.manualSendToStudent) {
            ...
            const base64InternalNotePdf = getBase64(options.P360_INTERNAL_NOTES.VARSEL_BLOCKED_ADDRESS); // convert internal note to base64
            const userEmailAddress = getEmailFromFile(pdf);
            const internalNoteData = {
                documentType: "INTERNT_NOTAT_VARSEL",
